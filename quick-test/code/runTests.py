# runTests.py: test out lots of combinations of runs (final submit step can be enabled/disabled)
'''
output of each test is a small set of generated files in the directory:
    submitted/target.mtype.stype.runs.nodes

where:
    target      - the name of the --target value
    mtype       - one of: hp-static, multi, hp-dyn, single
    stype:      - one of: random, dgd, bayesian, none
    runs:       - actual # of runs generated or "-1" for unlimited
    nodes:      - # of nodes specified for run
'''
import os
import re
import ast
...
if pc_utils.is_windows():
    APPROVED_DIR = "approved_{}_windows".format(SUBMIT_LOGS_DIR)
else:
    APPROVED_DIR = "approved_{}_linux".format(SUBMIT_LOGS_DIR)
...
def main(philly=1):
    ...
    tester = RunTests(config, seed, False, philly=philly)

    started = time.time()

    #tester.test_cmd("xt --target=local --runs=1 --nodes=1 --search-type=dgd --data-action=none --model-action=none --submit-logs=submitLogs/local.hp-args.dgd.1.1 --fake-submit=True run code/miniMnist.py --lr=[.01, .03, .05] --optimizer=[sgd, adam] --mlp-units=[64, 100, 128, 256]  ")
    tester.gen_all()
  
    elapsed = time.time() - started
    print("generated of {} tests took: {:.2f} secs".format(len(tester.all_cmds), elapsed))

    # scale this up to 100, 5
    fake_runs = 100
    actual_runs = 5
    tester.test_random_subset_cmds(fake_runs, actual_runs)

    print("all runTests passed")
    print("runs_count={}, file_count={}, assert_count={}".format(tester.cmd_count, tester.file_count, tester.assert_count))

    count = tester.file_count + tester.cmd_count
